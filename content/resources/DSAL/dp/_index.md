...
1. `dp[i]`: 数字`i`中`1`的个数
2. 如果`i`是奇数，则`dp[i]=dp[i-1]+1`， 因为`i`的二进制表达中末尾一定是1恰好比`dp[i-1]`的二进制表达多出1个1
3. 如果`i`是偶数, 不难发现其二进制表达中末尾一定是0，因此去掉该0对最后结果没有影响。而`i>>1`一定比`i`小，而且我们已经计算过了。
